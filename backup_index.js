...
(async () => {
  try {
    ...
    while (true) {
      ...
      await page.waitForSelector(".m-event-sport", {
        timeout: 3000
      });
      // Extract the links
    ...
    if(gameLinks.length > 0){
      ...
      await page.waitForSelector(".bet-count-wrapper", {
        timeout: 10000
      });
  
      await page.evaluate(() => {
        const items = document.querySelectorAll(".bet-count-wrapper");
        if (items) {
      ...
      await page.waitForSelector(".place-bet.bet-accept-change.bet-disabled", {
        timeout: 10000
      });
  
      await page.evaluate(() => {
        const items = document.querySelectorAll(
          ".place-bet.bet-accept-change.bet-disabled"
        );
        if (items) {
          for (let item of items) {
            item.click();
      ...
      await page.evaluate(() => {
        const items = document.querySelectorAll(".m-book-btn");
        if (items) {
          for (let item of items) {
            item.click();
            return true;
      ...
      await page.waitForSelector(".share-section-code", {
        timeout: 10000
      });
  
      await page.evaluate(() => {
        const items = document.querySelectorAll(".share-section-code");
        if (items) {
          const codeText = document
            .querySelector(".share-section-code")
            .textContent.trim()
            .replace(/\s+/g, "");
          console.log("SPORTYBET BOOKING CODE: ", codeText);
          return true;
        }
        return false;
      });
